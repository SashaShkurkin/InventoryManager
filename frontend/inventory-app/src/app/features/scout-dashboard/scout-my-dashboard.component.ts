...
@Component({
  selector: 'app-scout-my-dashboard',
  standalone: true,
  imports: [
    CommonModule,
    CurrencyPipe,
  ...
  template: `
    @if (loading()) {
      ...
      <div class="dashboard-wrap">
        <div class="scout-name">{{ data()!.scout.name }}</div>
        <div class="scout-tag">Scout Tag: {{ data()!.scout.tagId }}</div>
  ...
  styles: [`
    ...
    .scout-name { font-size: 1.5rem; font-weight: 600; margin-bottom: 4px; }
    .scout-tag  { color: #666; margin-bottom: 24px; }
...
export class ScoutMyDashboardComponent implements OnInit {
  loading = signal(true);
  ...
  ngOnInit(): void {
    this.svc.getMyDashboard().subscribe({
